...
if ! command -v mysqldump &>/dev/null; then
    ...
    if command -v apt-get &>/dev/null; then
        ...
        install_pkg mariadb 2>/dev/null \
            || install_pkg community-mysql 2>/dev/null \
            || install_pkg mariadb105 2>/dev/null \
            || install_pkg mysql-community-client \
            || fail "Could not find a MySQL/MariaDB client package. Install mysqldump manually."
    elif command -v apk &>/dev/null; then
